...
export default function AppStudioPage() {
  ...
  return (
    <Shell>
      <PageHeader
        title="App Studio"
        subtitle="Construye tu app o automatización con IA. La publicación es simulada (sin pasarela de pago configurada): no se cobra ni se despliega a un servidor real."
      />
      <div className="p-8">
            ...
            {!active ? (
              ...
              <>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  ) : active.status === "simulado" ? (
                    <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">Simulada (demo)</span>
                  ) : (
                ...
                {checkout && active.status !== "deployed" ? (
                  ...
                  <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                    {active.note || "Publicación simulada: no se cobró ni se desplegó a un servidor real."}
                    <button
                      onClick={() => deploy(active)}
                      disabled={busy}
                      className="mt-2 block rounded-lg border border-amber-300 px-3 py-1.5 text-xs font-semibold text-amber-800 disabled:opacity-50"
                    >
                      Volver a simular publicación
                    </button>
                  </div>
                ) : (
                  <button
                  ...
                  >
                    <Rocket className="h-4 w-4" /> Publicar (simulado)
                  </button>
                )}
              </>
            )}
              ...
              {apps.map((a) => (
                <div key={a.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${a.status === "deployed" ? "bg-emerald-100 text-emerald-700" : a.status === "simulado" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>
                      {a.status === "deployed" ? "Producción" : a.status === "simulado" ? "Simulada" : "Construida"}
                    </span>

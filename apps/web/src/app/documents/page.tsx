...
export default function DocumentsPage() {
  ...
  async function reindex() {
    ...
    try {
      ...
      setMsg(`✓ Re-indexado: ${r.documents} doc(s), ${r.chunks} fragmento(s)`
        + (r.orphans_purged ? `, ${r.orphans_purged} huérfano(s) purgado(s)` : "") + ".");
    } catch (e) { setMsg(e instanceof Error ? `✗ ${e.message}` : "✗ No se pudo re-indexar"); }
  ...
  async function retag(id: string, body: { area?: string; category?: string; sensitivity?: string }) {
    setMsg("");
    try { await api.updateDocument(id, body); load(); }
    catch (e) { setMsg(e instanceof Error ? `✗ ${e.message}` : "✗ No se pudo actualizar"); }
  }

  async function remove(id: string) {
    setConfirmDel(null); setMsg("");
    try {
      await api.deleteDocument(id);
      setMsg("✓ Documento borrado y quitado del índice RAG.");
      load();
    } catch (e) { setMsg(e instanceof Error ? `✗ ${e.message}` : "✗ No se pudo borrar"); }
  }
  ...
  return (
    <Shell>
      ...
      <div className="flex items-center justify-end gap-3 px-8 pt-4">
        {msg && <span className="rounded-lg bg-slate-100 px-3 py-1 text-xs text-slate-600">{msg}</span>}
        {confirmReindex ? (
          <span className="flex items-center gap-2 text-xs text-slate-600">
            ¿Reconstruir el RAG?
            <button onClick={reindex} disabled={reindexing} className="rounded-md bg-violet-600 px-2 py-1 font-semibold text-white disabled:opacity-50">Sí</button>
            <button onClick={() => setConfirmReindex(false)} className="rounded-md border border-slate-300 px-2 py-1">No</button>
          </span>
        ) : (
                      ...
                      <td className="px-4 py-2 text-right">
                        {confirmDel === d.id ? (
                          <span className="inline-flex items-center gap-2 text-xs">
                            ...
                            <button onClick={() => remove(d.id)} className="font-semibold text-red-600">Sí</button>
                            <button onClick={() => setConfirmDel(null)} className="text-slate-400">No</button>
                          </span>
                        ) : (
                          <button onClick={() => setConfirmDel(d.id)} className="text-slate-400 hover:text-red-600" title="Borrar (también del RAG)">

...
| **Canvas visual (read-only)** | ✅ Prod (Fase A) | `WorkflowCanvas` dibuja cada automatización como diagrama de nodos (Disparador→Entrada→Acción→Modelo→Salida) con semáforo de estado, reusando `/validate`. Marca blanca (el cliente no ve n8n). Fase B (editable) pendiente. |

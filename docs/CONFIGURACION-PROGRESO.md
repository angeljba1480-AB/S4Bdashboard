...
| **Canvas multi-paso (constructor)** | ✅ Prod (Fase 2) | `StepsCanvas`: botón **Constructor** en cada automatización → agrega/ordena (←→)/edita/elimina nodos del pipeline y guarda con `PUT /steps`. Cada tipo de paso tiene su editor (fetch/workflow/recipe/ai/connector/notify/deliver). |
